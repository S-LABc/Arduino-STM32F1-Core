...
HardwareTimer *tone_timer;           // timer used to generate frequency
...
// timer hander for tone with no duration specified, will keep going until noTone() is called
...
void tone_handler_2(void)
{
   ...
   if (--tone_n == 0)
   {
      ...
      tone_n = tone_ncount;
      if (!--tone_nhw)
      {                            // check if tone duration has finished
